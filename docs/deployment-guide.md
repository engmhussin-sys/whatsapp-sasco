...
**العرض:** البناء عبر Docker يتقدّم بنجاح (تثبيت الحزم، `prisma generate`
يظهر أنه يعمل) ثم يفشل عند `npm run build` بخطأ TypeScript يذكر أن Enum
معرَّف فعليًا في `schema.prisma` غير موجود في الأنواع المُولَّدة.

**السبب:** الـ `Dockerfile` كان يُنفّذ `npx prisma generate` كخطوة منفصلة
**قبل** نسخ بقية الكود المصدري (`COPY . .`)، ثم `npm run build` كخطوة
منفصلة لاحقة. هذا الترتيب صحيح نظريًا، لكن ثبت عمليًا أنه قد ينتج عميل
Prisma لا يعكس المخطط الكامل في بعض بيئات البناء — وهي مشكلة معروفة، ولذلك
توصي توثيقات Prisma الرسمية بنمط مختلف.

**الإصلاح المُطبَّق في `backend/Dockerfile` الآن:** نسخ الكود المصدري
**بالكامل أولًا** (`COPY . .`، شاملاً `prisma/schema.prisma`)، ثم تشغيل
`prisma generate` و`npm run build` **معًا في أمر واحد متسلسل**
(`npx prisma generate && npm run build`) بدلًا من خطوتين منفصلتين — هذا هو
النمط الموصى به رسميًا من Prisma لتفادي هذه الفئة من الأخطاء تمامًا.

**⚠️ ملاحظة شفافية مهمة:** لا أستطيع التحقق محليًا من أن هذا الإصلاح يحل
المشكلة **100%** لأن بيئة تطوير هذا المشروع لا تستطيع تشغيل `npx prisma
generate` فعليًا (نفس قيد `binaries.prisma.sh` المذكور سابقًا) — لذلك هذا
الخطأ تحديدًا **لا يمكن أن يظهر في فحوصاتي المحلية** (`tsc`/`jest`) مهما
كانت شاملة، لأنها تعتمد على Enum يدوي أُدرجه أنا يدويًا في stub مطابق
للـ schema، وليس على تشغيل `prisma generate` الحقيقي. الإصلاح مبني على
النمط الموصى به رسميًا من توثيق Prisma + السجل الفعلي الذي أرسلته، وليس
على تحقق مباشر مني. **يجب تأكيد نجاحه من خلال محاولة نشر فعلية جديدة.**
