...
**✅ تحديث مؤكَّد لاحقًا**: سجلّ Deploy Logs التالي أظهر
`The database is already in sync with the Prisma schema` — أي أن هذا
الإصلاح **نجح فعليًا**، وأصبح النشر يكتمل خلال ثوانٍ كما هو متوقَّع.

### المشكلة 9: `RUN_SEED=true` نجح في تشغيل السكربت، لكنه فشل بخطأين TypeScript

**السياق**: لتشغيل بيانات البذر الأولية (Seed) دون الحاجة لتنفيذ أوامر
مباشرة على الحاوية (أداة وكيل Railway لا تدعم ذلك)، أُضيفت خطوة اختيارية
في `start.sh` تُشغِّل `npm run prisma:seed` عند ضبط `RUN_SEED=true`. عند
التجربة الفعلية، وصل التنفيذ لمرحلة تشغيل السكربت فعليًا، لكنه فشل بخطأين:

```
error TS2307: Cannot find module '../src/modules/task-engine/task-field-type.enum'
error TS7016: Could not find a declaration file for module 'bcrypt'
```

**السبب الأول:** حاوية الإنتاج تنسخ `dist/` (الكود المُصرَّف) فقط، وليس
`src/` الأصلي. لكن `prisma/seed.ts` يُشغَّل مباشرة عبر `ts-node` (وليس عبر
`dist/`)، ويستورد من `src/` نسبيًا — وهو غير موجود في حاوية الإنتاج أصلًا.

**السبب الثاني:** `@types/bcrypt` في `devDependencies`، وaحاوية الإنتاج
تستبعدها (`npm ci --omit=dev`)، بينما `ts-node` يفحص الأنواع بالكامل
افتراضيًا فيحتاج كل حزم `@types/*` المُستخدَمة، حتى في الإنتاج — **نفس
فئة الخطأ التي واجهناها مرتين سابقًا** (`prisma` ثم `ts-node`/`typescript`
كأدوات CLI مفقودة من الإنتاج).

**الإصلاح الجذري (وليس ترقيعًا حزمة تلو الأخرى):**
1. نسخ `src/` و`tsconfig.json` أيضًا إلى حاوية الإنتاج (`Dockerfile`) — يحل السبب الأول
2. تشغيل `ts-node --transpile-only` بدل `ts-node` العادي في سكربتَي `prisma:seed` — يُلغي فحص الأنواع بالكامل وقت التشغيل، فلا حاجة لأي حزمة `@types/*` في الإنتاج **مطلقًا** بعد الآن — حل دائم لكل هذه الفئة من الأخطاء وليس فقط لـ `bcrypt`

**✅ تم التحقق فعليًا بأعمق طريقة ممكنة محليًا**: شغّلت `ts-node --transpile-only
prisma/seed.ts` محليًا فعليًا (وليس فحصًا نظريًا) — اختفى الخطآن المُحدَّدان
تمامًا. ثم اختبرت السكربت **كاملًا من البداية للنهاية** باستخدام محاكاة
وظيفية لقاعدة البيانات (تُحاكي كل استدعاء Prisma فعليًا)، واكتمل بنجاح
تام، طابعًا بيانات الاعتماد المتوقَّعة بالضبط. هذا أقوى تحقّق محلي تحقّق
في هذا المشروع لأي جزء يتعلّق بـ Prisma.
